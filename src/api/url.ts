const v1 = '/v1';

export const THEME_URL = `${v1}/theme`;
export const THEME_DETAIL_URL = `${v1}/theme/detail`;
export const THEME_LIST_URL = `${v1}/theme/list`;
export const THEME_APPROVE_URL = `${v1}/theme/approve`;
export const THEME_INSTANCE_URL = `${v1}/theme/instance`;

export const CONFIG_URL = `${v1}/config`;

export const CONFIG_APPROVE_URL = `${v1}/config/approve`;

export const CONFIG_APPROVE_PUBLISH_URL = `${v1}/config/approve/publish`;

export const CONFIG_APPROVE_DELETE_URL = `${v1}/config/approve/delete`;
export const VISIT_URL = `${v1}/visit`;
export const VISIT_APPROVE_URL = `${v1}/visit/approve`;

export const USER_URL = `${v1}/user`;
export const SOME_USER_AUTHORIZATION_URL = `${v1}/user/auth`;
export const USER_AUTHORIZATION_URL = `${v1}/auth`;
export const USER_INFO_URL = `${v1}/info`;

export const UPLOAD_PROTO_URL = `${v1}/upload/grpc`;
export const TEMPLATE_LIST_URL = `${v1}/template/list`;

const userV1 = '/user/v1';

export const LOGIN_URL = `${userV1}/login`;
